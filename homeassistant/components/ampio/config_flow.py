    ...
    async def async_step_user(
        ...
        if user_input is not None:
            ...
            if info is not None:
                ...
                # Backfill the Ethernet MAC (from DHCP discovery) onto the entry
                # so a future DHCP renewal can find it without prompting.
                extra: dict[str, Any] = (
                ...
                self._abort_if_unique_id_configured(
                    updates={CONF_HOST: user_input[CONF_HOST], **extra}
                )
                return self.async_create_entry(
                    title="Ampio", data={**user_input, **extra}
                )
