...
def discovery_skip_reason(obj: AmpioObject, sensor_kinds: frozenset[str]) -> str | None:
    ...
    Single source of truth for the sensor platform's discovery filter.
    ``sensor_kinds`` is the set of ``SensorKind.key`` values the platform has
    entity descriptions for.
    """
    if obj.kind is None:
        return "no_kind"
...
def module_device_info(
    ...
    info reply; every other module is linked back through ``via_device_id``,
    which requires the M-SERV device to already be registered.
    """
    ...
    # the HA device with what they see in those tools.
    ...
    if not is_mserv:
        ...
        info["via_device_id"] = async_get_device_id_by_identifier(
            coordinator.hass,
            (DOMAIN, f"{prefix}:{mserv_module.mac}"),
    ...
    elif ethernet_mac := coordinator.config_entry.data.get(CONF_MAC):
