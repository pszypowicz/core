...
async def test_setup_retries_when_discovery_never_completes(
    hass: HomeAssistant,
    mock_config_entry: MockConfigEntry,
    mock_aiomqtt: FakeMqttBroker,
) -> None:
    """An incomplete discovery cycle retries instead of blaming the account."""
    mock_aiomqtt.disable_discovery = True
    mock_config_entry.add_to_hass(hass)

    assert not await hass.config_entries.async_setup(mock_config_entry.entry_id)
    await hass.async_block_till_done()
    assert mock_config_entry.state is ConfigEntryState.SETUP_RETRY
    assert not hass.config_entries.flow.async_progress_by_handler("ampio")
...
async def test_setup_pre_registers_mserv_device(
    ...
    mserv = device_registry.async_get_device_by_identifier(
        (DOMAIN, f"{device_prefix(mock_config_entry)}:1"), mock_config_entry.entry_id
    )
