...
DEFAULT_DETAILS: list[dict[str, Any]] = [
    {
        ...
        # bit 0 + bit 37: a `params` value wider than 32 bits, with the hidden
        # flag (bit 4) clear so the object stays visible.
        "params": (1 << 37) | 1,
...
class FakeMqttBroker:
    ...
    def __init__(self) -> None:
        ...
        # When True, connecting seeds no discovery payloads at all, so the
        # client's initial discovery cycle never completes.
        self.disable_discovery: bool = False
...
def mock_aiomqtt(monkeypatch: pytest.MonkeyPatch) -> FakeMqttBroker:
    ...
    Patching ``aiomqtt.Client`` at its source covers both the runner's
    connection and the config-flow probe (``AmpioClient.test_connection``),
    and survives the library moving them between its own modules.
    ``AmpioClient.start`` defaults are also shortened so connect-timeout tests
    don't sit on the 15s production default.
    """
    ...
    # Bind shorter timeouts to start / fetch_rooms /
    # wait_for_initial_discovery via `partial` instead of wrapping each one
    # in a pass-through coroutine.
    monkeypatch.setattr("aiomqtt.Client", partial(FakeMqttClient, broker))
    ...
    monkeypatch.setattr("ampio_mqtt._connection._RECONNECT_BACKOFF_MAX", 0.2)
