...
async def test_module_with_mixed_rooms_has_no_suggested_area(
    ...
    device = device_registry.async_get_device_by_identifier(
        (DOMAIN, f"{_prefix(mock_config_entry)}:{MAC_MSENS}"),
...
async def test_device_serial_number_uses_override_can_mac(
    ...
    mserv = device_registry.async_get_device_by_identifier(
        (DOMAIN, f"{_prefix(mock_config_entry)}:{MAC_MSERV}"),
        mock_config_entry.entry_id,
    )
    assert mserv is not None
    assert mserv.serial_number == "0x1"

    sens = device_registry.async_get_device_by_identifier(
        (DOMAIN, f"{_prefix(mock_config_entry)}:{MAC_MSENS}"),
        mock_config_entry.entry_id,
    )
...
async def test_mserv_network_mac_connection(
    ...
    mserv = device_registry.async_get_device_by_identifier(
        (DOMAIN, f"{_prefix(entry)}:{MAC_MSERV}"), entry.entry_id
    )
    assert mserv is not None
...
async def test_value_push_for_non_entity_object_does_not_fan_out(
    hass: HomeAssistant,
    mock_config_entry: MockConfigEntry,
    mock_aiomqtt: FakeMqttBroker,
    entity_registry: er.EntityRegistry,
) -> None:
    """A value push for an object with no entity must not refresh other entities.

    Object 99 is a ghost (empty leafId) that never becomes an entity. Its
    structural fingerprint is recorded during setup, so a later value-only push
    matches it and is short-circuited before the discovery fan-out. Without the
    gate the catchall would re-write every entity; ``last_reported`` advances on
    any write (even when the value is unchanged), so it is the signal that
    catches a spurious fan-out.
    """
    await _setup(hass, mock_config_entry)
    ...
    temp_before = hass.states.get(temp_id).last_reported

    mock_aiomqtt.push_state(99, "5.0")
    await hass.async_block_till_done()

    assert hass.states.get(temp_id).last_reported == temp_before
...
async def test_orphan_object_is_filtered(
    ...
    assert (
        device_registry.async_get_device_by_identifier(
            (DOMAIN, f"{_prefix(mock_config_entry)}:hub"), mock_config_entry.entry_id
        )
        is None
    )
